...
describe '/files/download_setup' do
  ...
  context 'simple request' do
    ...
    before { 
      NodeApi::Transfer.any_instance.stub(:download_token).and_return("abc")
      post node_api_files_download_setup_path, request_body 
    }
          ...
          "transfer_specs"=>[
            {
              "transfer_spec"=>{
                "destination_root"=>"/destination/root", 
                "source_root"=>"/source/root", 
                "paths"=>[{"source"=>"/foo.1"}, {"source"=>"/foo.2"}], 
                "token"=>"abc", 
                "direction"=>"send",
                "remote_host"=>"example.org", 
                "remote_user"=>"asp1", 

...
describe LoginEvent do
  ...
  let(:event) { 
    set_thread_globals(authorizable_user)
    LoginEvent.create_event
  }
  ...
  describe '#process' do
    ...
    describe 'user feed' do
      ...
      it "creates user feed items" do
        user_feed_item.event_type_name.should == 'Login'
        user_feed_item.description.should == 'You logged in from 1.2.3.4'
      end
    end
    ...
    describe 'admin directory feed' do
      ...
      it "creates directory feed item" do
        dir_feed_item.event_type_name.should == 'Login'
        dir_feed_item.description.should == "User '#{authorizable_user.username}' logged in from 1.2.3.4"
      end
    end
